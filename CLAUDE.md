...
**Callsign format**: a callsign is an optional prefix plus a number — `325`, `D-325`, `SWAT-01` — parsed by `parseCallsign` (mirrored in app.js). The prefix **must contain a non-digit**: without that rule `"325"` splits into prefix `3` + number `25`, and any range crossing a hundred (95→105) reads as two different series and generates nothing. Both ends of a range must carry the *exact* same prefix, because generation emits `start.prefix` verbatim. Callsigns are unique **case-insensitively** (`occupiedCallsignConflict`, `overlappingRankBlocks`, and the slot generator's `taken` set all lower-case their keys), so `D-325` and `d-325` are one slot and can't be created twice or claimed by two ranks. Sort with `byCallsign` — prefix first, then number, so `D-9` precedes `D-10`.
